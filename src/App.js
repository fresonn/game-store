...
const App = props => {
    ...
    /*
        ВАЖНО!
        Иногда авторизация не будет происходить автоматом, т.к firebase дает маленькую квоту на проверку/отслеживание токена.
        Это значит, что в ответе на отправленный токен может прийти expiresIn текущее время, которое конечно не пройдет проверку в action авторизации(время просто будет совпадать во время сравнения)
    */
    useEffect(() => {
        authWithToken()
    }, [authWithToken])
